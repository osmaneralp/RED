...
//#include <avr/io.h>
#include <variant.h>
//#include <C:\Program Files (x86)\Arduino\hardware\arduino\sam\system\libsam\include\pio.h>
#if ARDUINO > 22
  #include "Arduino.h"
#else
  #include "WProgram.h"
...
byte PS2X::config_gamepad() {
   return config_gamepad(false, false);
}

/****************************************************************************************/
byte PS2X::config_gamepad(bool pressures, bool rumble) {
  ...
  _clk_mask = digitalPinToBitMask(clk);
  lport = (uint32_t)digitalPinToPort(clk);
